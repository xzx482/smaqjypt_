		...
		if(访问_暂停 and 访问_暂停_):
			...
			#'''
				...
				try:
					...
					错误_计数+=1
					if(安静模式_):
						print(" 网络错误:"+str(错误)+","+str(错误_计数)+"次")
						if(错误_计数>5):
							按任意键继续(" 网络错误:"+str(错误)+",共"+str(错误_计数)+"次,",提示信息_3='再次重试')
						else:
							time.sleep(3)
					else:
						按任意键继续(" 网络错误:"+str(错误)+",",提示信息_3='重试')
					访问_暂停_=False
	#"""
	#响应=request.urlopen(请求)
...
'''
	...
	for i in 用户名+密码:
		if(not i in url字符):
			return ["",{'Code':-1}]
	访问结果=访问("https://fujianlogin.xueanquan.com/LoginHandler.ashx?userName="+用户名+"&password="+密码+"&type=login&loginType=1",{"referer":"https://sanming.xueanquan.com/login.html"})
	返回数据=json.loads(访问结果[6])#.replace("data:{","").replace("ret: 1,","").replace("{","").replace("}","").replace("(","").replace(")","").split(",")
	#返回数据=访问结果[6].replace("data:{","").replace("ret: 1,","").replace("{","").replace("}","").replace("(","").replace(")","").split(",")
	用户信息_={"Code":返回数据["Code"],"ErrorMsg":返回数据["ErrorMsg"],"ErrorNum":返回数据["ErrorNum"]}
	try:
		用户信息_.update(返回数据["UInfo"])
	except:
		pass
	cookie=访问结果[4]
	...
	#	用户信息_[index[0]]=index[1]
	#input(cookie)
	#print(用户信息_["Code"])
	if(用户信息_['Code']==1 and 保存用户信息):
		if(isinstance(临时用户数据,dict)):
			临时用户数据[用户名]=[cookie,用户信息_]
		else:
			try:
				try:
...
参数:
	grade:
		年级,"登录"得到的"用户信息_"中的"grade"
	
	classroom:
		班级,"登录"得到的"用户信息_"中的"classroom"
	
	citycode:
		城市代码,"登录"得到的"用户信息_"中的"citycode"
...
'''
	访问结果=访问("https://file.safetree.com.cn/webapi.fujian/jt/MyHomeWork.html?grade="+str(grade)+"&classroom="+str(classroom)+"&cityid="+str(citycode))
...
'''
	#print(用户信息_)
	所有作业=查询所有作业(用户信息_["Grade"],用户信息_["ClassRoom"],用户信息_["CityCode"])
...
def 完成安全学习(cookie,用户信息_,workid,showhdtcbox):
	'''
...
'''
	...
	访问("https://sanming.xueanquan.com/jiating/ajax/FamilyEduCenter.EscapeSkill.SeeVideo,FamilyEduCenter.ashx?_method=AllSituation&_session=rw",请求头,"POST","gradeid="+str(用户信息_["Grade"]))
	#print(.decode("utf-8"))
...
def 获取专题id_网页(url,id="specialId",跳转=0):
	'''
"获取专题id"会调用
...
参数:
	...
	id:
		要获取的id,默认是specialId(专题id,普通专题使用),常用的还有schoolYear(年,寒暑假专题使用)
...
'''
	...
	if("window.location.href" in 返回数据 and "<title>跳转中...</title>" in 返回数据):
		...
		if(url2[-11:]=="/index.html"):
			try:return 获取专题id_网页(url2[0:-11]+"/huodong.html",id)
			except:return 获取专题id_网页(url2[0:-11]+"/shipin.html",id)
...
def 完成专题活动(cookie,用户信息_,specialId):
	'''
...
'''
	...
	for index in 数据:
		...
		if(str(type(数据[index]))=="<class 'list'>"):
			数据_替换=[]
			...
			返回数据=访问(专题问卷url,请求头,"POST",json.dumps(
				{"user":
					{"userID":"0",
					"userName":用户信息_["UserName"],
					"trueName":用户信息_["TrueName"],
					"regionalAuthority":用户信息_["regionalAuthority"],
					"userType":"Users",
					"prvCode":用户信息_["PrvCode"],
					"cityCode":用户信息_["CityCode"],
					"schoolId":用户信息_["SchoolID"],
					"schoolName":用户信息_["SchoolName"],
					"grade":用户信息_["Grade"],
					"classRoom":用户信息_["ClassRoom"],
					"comeFrom":用户信息_["ComeFrom"]},
					"UserAnswers":数据_替换,
					"specialId":specialId,
					"step":index
				}
...
def 完成寒暑假专题活动(cookie,用户信息_,专题布置时间):
	'''
...
'''
	...
	#try:''
		...
		if(not 安静模式):
			print('专题未记录,从github获取')
		数据=获取文件("%E6%95%B0%E6%8D%AE/"+专题布置时间+".json","json")
		if(数据 is None):
			if(not 安静模式):
				print('获取失败')
			return 0
	...
	专题点击url="https://huodongapi.xueanquan.com/p/fujian/Topic/topic/platformapi/api/v1/holiday/sign"#注意,此处的url与"完成专题活动"的url不同
	专题问卷url="https://huodong.xueanquan.com/HolidayService/SubmitTest"
	for index in 数据["步骤"]:
		返回数据=访问(专题点击url,请求头,"POST",'{"schoolYear":'+数据["schoolYear"]+',"semester":'+数据["semester"]+',"step":'+index+',"prvName":"'+用户信息_["PrvName"]+'","cityName":"'+用户信息_["CityName"]+'"}')[6]
		if(str(type(数据["步骤"][index]))=="<class 'list'>"):
			...
			返回数据=访问(专题问卷url,{**请求头,**{"Content-Type":"application/x-www-form-urlencoded"}},"POST",
				parse.urlencode({
					...
					'userType':用户信息_["UserType"],
					'answerJson':json.dumps(数据["步骤"][index],ensure_ascii=False),
					'prv':用户信息_["PrvCode"],
					'city':用户信息_["CityCode"],
					'county':用户信息_["CountryId"],
					'school':用户信息_["SchoolID"],
					'grade':用户信息_["Grade"],
					'Class':用户信息_["ClassRoom"],
					'comefrom':用户信息_["ComeFrom"],
					'version':数据["version"],
					'TrueName':用户信息_["TrueName"],
					'prvName2':用户信息_["PrvName"],
					'cityName2':用户信息_["CityName"],
				})
			)[6]
		#if(not 返回数据["result"]):
		#	完成状态=0
...
注意,安全教育平台增加了次数限制,因此 尝试简单密码 功能已取消
...
'''
	...
	while((not 自动)and 用户信息[1]['Code']!=1):
		if(用户名!=""):
			自动=True
		if(not 自动):
			#print('-'*18)
		...
		if(not 密码):
				...
				for index in 弱密码列表:
					...
					if(用户信息[1]['Code']==1):
						if(not 安静模式_):
							print("登录成功\t\t")
						break
				if(用户信息[1]['Code']!=1 and not 安静模式_):
					print("登录失败\t\t")
			else:
				if(not 安静模式_):
					print("使用已保存的用户信息")
			...
			if(用户信息[1]['Code']==-1):
				if(安静模式_):
					...
					print("用户名或密码错误")
			elif(用户信息[1]['Code']==1 and not 安静模式_):
				print("登录成功")
			elif(not 安静模式_):
				print("未知错误")
...
def 自动完成(用户信息=["",{'Code':-1}],自动=False):
	'''
快速完成用户在 三明安全教育平台 中的所有未完成作业

参数:
	用户信息:
		"登录"时得到的列表,即[cookie,用户信息_]
...
'''
	...
	for 作业id in 作业:
		if(not 作业[作业id][3]and 作业[作业id][4][4]!=0):
			...
			if(安静模式):
				if(返回数据!=1):
					未完成信息添加(str(作业[作业id][0]),str(返回数据))
			else:
				if(返回数据==1):
					print("已完成\""+作业[作业id][0]+"\"\t\t\t\t\t\t")
				else:
...
def 使用用户名密码完成__(用户名,密码,总数):
	'''
"使用用户名密码表完成"函数调用时多线程使用会调用,不建议单独调用
...
'''
	...
	if(打印未完成):
		...
		if(未完成信息):
			print("在完成期间,发生了错误:")
			for 信息 in 未完成信息:
				print('\t\''+信息+'\''+str(未完成信息[信息][0])+'次,其中:',end='')
				for 内容 in 未完成信息[信息][1]:
					print('\''+内容+'\''+str(未完成信息[信息][1][内容])+'次',end='')
				print()

	#未完成表.sort()
	安静模式=安静模式2
	安静模式_=安静模式2_
	return [未完成表,未完成表_其他错误,未完成表_密码错误,未完成信息]
...
'''
	try:
		数据=访问("https://xzx482.gitee.io/smaqjypt_g/"+路径)[6]
	except:
		try:
...
if __name__ == '__main__':
	...
	更新url=[
		"https://xzx482.gitee.io/smaqjypt_g/",
		"https://raw.githubusercontent.com/xzx482/smaqjypt_/main/"
	]
	i=0
	while(i<len(更新url)):
		try:
			版本更新_获取=request.urlopen(request.Request(更新url[i]+"%E6%95%B0%E6%8D%AE/%E6%9C%80%E6%96%B0%E7%89%88%E6%9C%AC.json",headers={"User-Agent":"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/88.0.4324.150 Safari/537.36 Edg/88.0.705.63"}),timeout=8)
			i+=1
